...
import {
    ...
    MASTER_PLATYPUSV4_ABI,
    MASTER_PLATYPUSV4_ADDRESS,
    PTP_VOTER_ABI,
} from "./constants.js";
...
export const getYyPTPAPR = async () => {
    ...
    console.log(`Latest block has been retrieved, number = ${BigNumber.from(latestBlock).toNumber()}, hexValue = ${latestBlock}`);
    //
    // Next step is to get the voter contract for MasterPlatypusV4 which contains a lot of important info.
    //
    console.log("Getting the voter contract for MasterPlatypusV4...");
    const voterContract = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'voter', [], latestBlock);
    console.log(`voterContract = ${voterContract}`);
    //
    // Now we get some information from the voterContract.
    //
    console.log("Getting ptpPerSec and totalWeight...");
    let ptpPerSec = await callContractFunction(PTP_VOTER_ABI, voterContract, 'ptpPerSec', [], latestBlock);
    ...
    let totalWeight = await callContractFunction(PTP_VOTER_ABI, voterContract, 'totalWeight', [], latestBlock);
    totalWeight = convertWithDecimals(BigNumber.from(totalWeight).toString(), 18);
    console.log(`totalWeight = ${totalWeight}`);
    //
    // Now we need the poolLength so that we can call poolInfo and userInfo functions for all pools.
    //
    console.log("Getting the poolLength for MasterPlatypusV4 contract...");
    let poolLength = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'poolLength', [], latestBlock);
    poolLength = BigNumber.from(poolLength).toNumber();
    console.log(`poolLength = ${poolLength}`);
    //
    // Now we get the diluting and non-diluting repartitions, which represent the share of PTP emissions going to "base" rewards
    ...
    //
    console.log("Getting the dilutingRepartition and nonDilutingRepartition for MasterPlatypusV4 contract...");
    let dilutingRepartition = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'dilutingRepartition', [], latestBlock);
    dilutingRepartition = BigNumber.from(dilutingRepartition).toNumber();
    let nonDilutingRepartition = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'nonDilutingRepartition', [], latestBlock);
    nonDilutingRepartition = BigNumber.from(nonDilutingRepartition).toNumber();
    console.log(`dilutingRepartition = ${dilutingRepartition}, nonDilutingRepartition = ${nonDilutingRepartition}`);
    //
    ...
    //
    const poolIDs = [...Array(poolLength).keys()];
    console.log(`Getting the poolInfo and userInfo for MasterPlatypusV4 contract for our ${poolLength} pools...`);
    const poolUserInfo = await Promise.all(poolIDs.map(
        async (poolID) => {
            const poolInfo = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'poolInfo', [poolID], latestBlock);
            let { lpToken, sumOfFactors } = poolInfo;
            ...
            // Now we get the weight from our voterContract.
            let weight = await callContractFunction(PTP_VOTER_ABI, voterContract, 'weights', [lpToken], latestBlock);
            weight = convertWithDecimals(BigNumber.from(weight).toString(), 18);
            // Get the totalSupply for the base proportion.
            let totalLpSupply = await callContractFunction(LP_TOKEN_ABI, lpToken, 'balanceOf', [MASTER_PLATYPUSV4_ADDRESS], latestBlock);
            // Platypus LP tokens have 6 decimals.
            totalLpSupply = convertWithDecimals(BigNumber.from(totalLpSupply).toString(), 6);
            const userInfo = await callContractFunction(MASTER_PLATYPUSV4_ABI, MASTER_PLATYPUSV4_ADDRESS, 'userInfo', [poolID, YYPTP_ADDRESS], latestBlock);
            let { amount, factor } = userInfo;
            ...
            return (
                {
                    poolID,
                    basePTPEarnedPerYear: weight === 0 ? 0 : ptpPerYear * weight * (dilutingRepartition / 1000) * amount / (totalWeight * totalLpSupply),
                    boostedPTPEarnedPerYear: weight === 0 ? 0 : ptpPerYear * weight * (nonDilutingRepartition / 1000) * factor / (totalWeight * sumOfFactors),
                }
            )
        }
    ));
